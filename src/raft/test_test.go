...
func TestFollowerFailure2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	DPrintf("断开连接 %v", (leader1+1)%servers)
	...
	leader2 := cfg.checkOneLeader()
	DPrintf("断开连接 %v", (leader2+1)%servers)
	cfg.disconnect((leader2 + 1) % servers)
	DPrintf("断开连接 %v", (leader2+2)%servers)
...
func TestLeaderFailure2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	DPrintf("leader1 %v 断开连接", leader1)
	cfg.disconnect(leader1)
	...
	leader2 := cfg.checkOneLeader()
	DPrintf("leader2 %v 断开连接", leader2)
	cfg.disconnect(leader2)
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	DPrintf("Old leader last commit %v logs%v", cfg.rafts[leader1].commitIndex, cfg.rafts[leader1].log)
	DPrintf("Test (2B): rejoin of partitioned leader: leader %v disconnected\n", leader1)
	...
	cfg.connect(leader1)
	DPrintf("Old leader last commit %v logs%v", cfg.rafts[leader1].commitIndex, cfg.rafts[leader1].log)
	DPrintf("Test (2B): rejoin of partitioned leader: leader %v reconnected\n", leader1)
...
func TestPersist32C(t *testing.T) {
	...
	leader := cfg.checkOneLeader()
	DPrintf("%v 断开连接", (leader+2)%servers)
	cfg.disconnect((leader + 2) % servers)

	cfg.one(102, 2, true)

	DPrintf("%v 断开连接", (leader+0)%servers)
	cfg.crash1((leader + 0) % servers)
	DPrintf("%v 断开连接", (leader+1)%servers)
	cfg.crash1((leader + 1) % servers)
	DPrintf("%v 重新连接", (leader+2)%servers)
	cfg.connect((leader + 2) % servers)
	cfg.start1((leader+0)%servers, cfg.applier)
	DPrintf("%v 重新连接", (leader+0)%servers)
